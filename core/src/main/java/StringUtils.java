...
public class StringUtils {
	...
	public static String safeJsonString(final String s) {
		...
		for (int i = 0; i < len; i++) {
			...
			sb.append(new char[] { '\\', 'u', DIGITS[c >>> 12], DIGITS[(c >>> 8) & 0xf], DIGITS[(c >>> 4) & 0xf],
					DIGITS[c & 0xf] });
			copyIdx = i + 1;
		}
		if (sb == null)
	...
	public static void escapeHtmlRawInPlace(final StringBuilder sb, final char[] sArr, int start, final int end) {
	...
	public static final int levenshtein(final String s1, final String s2) {
		...
		for (int i = 0; i < len1; i++) {
			...
			for (int j = 0; j < len2; j++)
				v1[j + 1] = Math.min(c1[i] == c2[j] ? v0[j] : v0[j] + 1, // substitute
