package scalalib
package cuteName

// children friendly names only
// this is used by /class
object CuteNameGenerator:

  type Language = String
  type UserName = String
  val defaultLang: Language = "en"

  private type Combinations = Vector[List[Vector[String]]]

  def make(maxSize: Int = 20, triesLeft: Int = 100, lang: Language = defaultLang): Option[UserName] =
    val name = makeForSure(lang)
    if name.sizeIs <= maxSize then Some(name)
    else if triesLeft <= 0 then None
    else make(maxSize, triesLeft - 1, lang)

  def makeForSure(lang: Language): UserName =
    anyOf(getCombinations(lang)).map(anyOf).mkString

  def fromSeed(seed: Int, lang: Language = defaultLang): UserName =
    seedOf(seed)(getCombinations(lang)).map(seedOf(seed)).mkString

  private def anyOf[A](vec: Vector[A]): A =
    vec(ThreadLocalRandom.nextInt(vec.size))

  private def seedOf[A](seed: Int)(vec: Vector[A]): A =
    vec(Math.abs(seed) % vec.size)

  private def getCombinations(lang: Language): Combinations =
    combinations.getOrElse(lang, defaultCombinations)

  private lazy val defaultCombinations =
    import CuteNameDicts.en.*
    val beings = animals ++ pieces ++ CuteNameDicts.any.players
    Vector(
      List(colors ++ adjectives, beings ++ jobs),
      List(colors, adjectives, beings ++ jobs),
      List(colors ++ adjectives, adjectives, beings ++ jobs),
      List(colors ++ adjectives, jobs, beings)
    )

  private lazy val combinations: Map[Language, Combinations] = Map(
    defaultLang -> defaultCombinations,
    "fr" -> {
      import CuteNameDicts.fr.*
      val beings = animals ++ pieces ++ CuteNameDicts.any.players
      Vector(
        List(adjectives.pre, beings),
        List(adjectives.pre, beings, colors),
        List(adjectives.pre, beings, adjectives.post),
        List(beings, colors),
        List(beings, adjectives.post),
        List(beings, colors, adjectives.post)
      )
    },
    "pt" -> {
      import CuteNameDicts.pt.*
      val beings = animals ++ pieces ++ CuteNameDicts.any.players
      Vector(
        List(beings, adjectives ++ jobs ++ colors),
        List(beings, adjectives, colors),
        List(beings, jobs, adjectives ++ colors),
        List(beings, jobs)
      )
    }
  )

/*
 * Lichess usernames must only have latin characters, or it would become very hard for other people to type them.
 * To contribute a new language, modify
 * https://github.com/lichess-org/lila/blob/master/modules/common/src/main/CuteNameDicts.scala
 * and, if you can,
 * https://github.com/lichess-org/lila/blob/master/modules/common/src/main/CuteNameGenerator.scala#L45.
 */
private object CuteNameDicts:
  object any:
    def players = Vector(
      "Alekhine",
      "Anand",
      "Botvinik",
      "Capablanca",
      "Carlsen",
      "Damiano",
      "Euwe",
      "Fischer",
      "Greco",
      "Hikaru",
      "HouYifan",
      "JuditPolgar",
      "Karpov",
      "Kasparov",
      "Kramnik",
      "Lasker",
      "Maric",
      "Petrossian",
      "Philidor",
      "RuyLopez",
      "Spassky",
      "Steinitz",
      "Symyslov",
      "Tal"
    )
  object fr:
    def pieces = Vector("Cavalier", "Dame", "Fou", "Pion", "Roi", "Tour")
    object adjectives:
      def pre =
        Vector(
          "Beau",
          "Bon",
          "Grand",
          "Large",
          "Long",
          "Petit",
          "Vrai",
          "Parfait",
          "Noble",
          "Sublime",
          "Pur",
          "Formidable",
          "Fabuleux"
        )
      def post = Vector(
        "Abondant",
        "Absolu",
        "Academique",
        "Acceptable",
        "Accueillant",
        "Actif",
        "Adequat",
        "Admire",
        "Admissible",
        "Adorable",
        "Adore",
        "Adroit",
        "Affable",
        "Affluent",
        "Agile",
        "Agreable",
        "Aigu",
        "Aile",
        "Aimable",
        "Aimant",
        "Aime",
        "Ajuste",
        "Alerte",
        "Allie",
        "Alphabetise",
        "Ameliore",
        "Amical",
        "Amoureux",
        "Amusant",
        "Angelique",
        "Anime",
        "Appetissant",
        "Approprie",
        "Apte",
        "Ardent",
        "Argente",
        "Arrondi",
        "Artistique",
        "Aspirant",
        "Assez",
        "Astucieux",
        "Athletique",
        "Attendu",
        "Attentif",
        "Attrayant",
        "Atypique",
        "Audacieux",
        "Authentique",
        "Avance",
        "Avide",
        "Beatifique",
        "Benefique",
        "Beni",
        "Benin",
        "Bien",
        "Bienheureux",
        "Bienveillant",
        "Bijou",
        "Brillant",
        "Bronze",
        "Brulant",
        "Calin",
        "Calmant",
        "Calme",
        "Capable",
        "Capital",
        "Carre",
        "Categorique",
        "Celebre",
        "Celeste",
        "Central",
        "Cerebral",
        "Cerise",
        "Certain",
        "Chamois",
        "Champion",
        "Chanceux",
        "Charmant",
        "Chaud",
        "Cheri",
        "Chevaleresque",
        "Chevronne",
        "Chic",
        "Chimerique",
        "Clair",
        "Classique",
        "Clement",
        "Coherent",
        "Colossal",
        "Comique",
        "Complet",
        "Compose",
        "Concentre",
        "Concis",
        "Confiant",
        "Confortable",
        "Connaissable",
        "Conscient",
        "Constant",
        "Content",
        "Convaincant",
        "Convenable",
        "Cool",
        "Copieux",
        "Cordial",
        "Correct",
        "Cosmique",
        "Courageux",
        "Courant",
        "Courtois",
        "Creatif",
        "Credible",
        "Croustillant",
        "Crucial",
        "Cultive",
        "Curieux",
        "Dandy",
        "Debonnaire",
        "Debordant",
        "Debout",
        "Decent",
        "Decisif",
        "Decision",
        "Defendable",
        "Delicat",
        "Desinteresse",
        "Desinvolte",
        "Desireux",
        "Detendu",
        "Didactique",
        "Difficile",
        "Digne",
        "Diligent",
        "Direct",
        "Discret",
        "Distinct",
        "Distingue",
        "Divers",
        "Divin",
        "Dominant",
        "Donnant",
        "Doue",
        "Doux",
        "Droit",
        "Drole",
        "Durable",
        "Dynamique",
        "Eblouissant",
        "Eclair",
        "Eclat",
        "Eclectique",
        "Economie",
        "Economique",
        "Ecurie",
        "Edifie",
        "Egal",
        "Electrique",
        "Elegant",
        "Eleve",
        "Eloquent",
        "Embrase",
        "Emergent",
        "Eminent",
        "Emouvant",
        "Empile",
        "Energique",
        "Engageant",
        "Enjoue",
        "Enracine",
        "Ensoleille",
        "Enthousiaste",
        "Entier",
        "Epanoui",
        "Epice",
        "Epris",
        "Equilibre",
        "Equipe",
        "Equitable",
        "Erudit",
        "Espiegle",
        "Esthetique",
        "Estime",
        "Estival",
        "Eternel",
        "Ethique",
        "Etoile",
        "Etouffant",
        "Etourdi",
        "Euphorique",
        "Eveille",
        "Evident",
        "Exact",
        "Exaltant",
        "Exalte",
        "Excentrique",
        "Exotique",
        "Expert",
        "Exultant",
        "Fabuleux",
        "Facile",
        "Factuel",
        "Faisable",
        "Familial",
        "Familier",
        "Fantaisie",
        "Favorise",
        "Ferme",
        "Fervent",
        "Fiable",
        "Fidele",
        "Fier",
        "Filaire",
        "Flottable",
        "Formidable",
        "Fort",
        "Fougueux",
        "Frais",
        "Franc",
        "Frappant",
        "Fraternel",
        "Fringant",
        "Fructueux",
        "Gagnant",
        "Galant",
        "Genereux",
        "Genial",
        "Gentil",
        "Glorieux",
        "Gracieux",
        "Gracile",
        "Grandiose",
        "Gratuit",
        "Gresillant",
        "Grouillant",
        "Guilleret",
        "Habile",
        "Haussier",
        "Heroique",
        "Heureux",
        "Honnete",
        "Honoraire",
        "Honore",
        "Humble",
        "Humoristique",
        "Hygienique",
        "Hypnotique",
        "Ideal",
        "Idolatre",
        "Imitable",
        "Immense",
        "Immortel",
        "Impartial",
        "Impeccable",
        "Incisif",
        "Inclus",
        "Incroyable",
        "Indemne",
        "Indigene",
        "Inebranlable",
        "Infaillible",
        "Infatigable",
        "Infini",
        "Informe",
        "Ingenieux",
        "Inhabituel",
        "Ininterrompu",
        "Innocent",
        "Inoffensif",
        "Insouciant",
        "Inspire",
        "Instruit",
        "Integral",
        "Intelligent",
        "Intemporel",
        "Intense",
        "Intime",
        "Intrepide",
        "Invaincu",
        "Jazzy",
        "Jeune",
        "Joli",
        "Joueur",
        "Jovial",
        "Joyeux",
        "Juste",
        "Laureat",
        "Legitime",
        "Leonin",
        "Lettre",
        "Liberal",
        "Libre",
        "Lisible",
        "Lisse",
        "Litteraire",
        "Logique",
        "Louable",
        "Loufoque",
        "Lourd",
        "Loyal",
        "Lucide",
        "Luisant",
        "Lumineux",
        "Lustre",
        "Luxuriant",
        "Magicien",
        "Magique",
        "Magistral",
        "Magnetique",
        "Magnifique",
        "Maitre",
        "Majestueux",
        "Majeur",
        "Manifeste",
        "Manifestement",
        "Maternel",
        "Mature",
        "Maximale",
        "Maximum",
        "Mediat",
        "Meilleur",
        "Melodieux",
        "Merveilleux",
        "Meteorique",
        "Mielle",
        "Mieux",
        "Mignon",
        "Misericordieux",
        "Modele",
        "Moderne",
        "Modeste",
        "Moelleux",
        "Mondain",
        "Morale",
        "Moteur",
        "Moulant",
        "Mouvemente",
        "Muscle",
        "Musculeux",
        "Musical",
        "Mutuel",
        "National",
        "Naturel",
        "Necessaire",
        "Nerveux",
        "Noble",
        "Normale",
        "Notable",
        "Nouveau",
        "Occasionnel",
        "Occupe",
        "Officiel",
        "Oisillon",
        "Olympien",
        "Opportun",
        "Optimal",
        "Optimiste",
        "Optimum",
        "Opulent",
        "Ordonne",
        "Organique",
        "Oriente",
        "Original",
        "Pacifique",
        "Paillete",
        "Parfait",
        "Parfume",
        "Particulier",
        "Partisan",
        "Passionne",
        "Patient",
        "Peche",
        "Peluche",
        "Pepere",
        "Persuasif",
        "Pertinent",
        "Petillant",
        "Pieux",
        "Pilote",
        "Pimpant",
        "Piquant",
        "Pittoresque",
        "Pivot",
        "Placide",
        "Plaisant",
        "Plenier",
        "Plus",
        "Poetique",
        "Poignant",
        "Pointu",
        "Poli",
        "Ponctuel",
        "Populaire",
        "Positif",
        "Possible",
        "Pratique",
        "Precieux",
        "Precis",
        "Prefere",
        "Premier",
        "Prepare",
        "Present",
        "Preserve",
        "Pret",
        "Primaire",
        "Prime",
        "Primitif",
        "Princier",
        "Principal",
        "Probable",
        "Proche",
        "Profond",
        "Prolifique",
        "Prospere",
        "Prudent",
        "Puissant",
        "Pur",
        "Qualifie",
        "Radieux",
        "Raffine",
        "Rapide",
        "Rare",
        "Rationnel",
        "Ravie",
        "Realisable",
        "Recherche",
        "Recompense",
        "Reconnaissant",
        "Regnant",
        "Regulier",
        "Remunerateur",
        "Renomme",
        "Reposant",
        "Resolu",
        "Respectueux",
        "Reveur",
        "Riche",
        "Risible",
        "Robuste",
        "Romantique",
        "Rond",
        "Rose",
        "Royal",
        "Ruse",
        "Sacre",
        "Sage",
        "Saillant",
        "Saint",
        "Saisonnier",
        "Salutaire",
        "Sanguine",
        "Sapide",
        "Savoureux",
        "Scenique",
        "Securise",
        "Seduisant",
        "Seigneurial",
        "Sensationnel",
        "Sensible",
        "Serein",
        "Serieux",
        "Simple",
        "Sincere",
        "Singulier",
        "Snob",
        "Sociable",
        "Sociale",
        "Societal",
        "Soigne",
        "Solide",
        "Somptueux",
        "Sorcier",
        "Sortant",
        "Soulage",
        "Souple",
        "Sourire",
        "Soyeux",
        "Spacieux",
        "Special",
        "Spirituel",
        "Splendide",
        "Sportif",
        "Standard",
        "Stellaire",
        "Stoique",
        "Studieux",
        "Suave",
        "Sublime",
        "Substantiel",
        "Subtil",
        "Succinct",
        "Succulent",
        "Sucre",
        "Super",
        "Superbe",
        "Superieur",
        "Supreme",
        "Sur",
        "Svelte",
        "Sympathique",
        "Talentueux",
        "Tangible",
        "Terrestre",
        "Titanesque",
        "Tolerant",
        "Tranquille",
        "Trapu",
        "Tubulaire",
        "Ultime",
        "Ultra",
        "Uni",
        "Unifie",
        "Unique",
        "Urbain",
        "Utile",
        "Utilisable",
        "Vaillant",
        "Valeureux",
        "Valide",
        "Vaste",
        "Vehement",
        "Venere",
        "Veridique",
        "Verifie",
        "Vertueux",
        "Viable",
        "Vibrant",
        "Vif",
        "Vigilant",
        "Vigoureux",
        "Vital",
        "Vivant",
        "Vocal",
        "Volant",
        "Zele"
      )
    def animals = Vector(
      "Abeille",
      "Aigle",
      "Alligator",
      "Alouette",
      "Alpaga",
      "Anguille",
      "Antilope",
      "Babouin",
      "Baleine",
      "Belette",
      "Bison",
      "Bouvreuil",
      "Buffle",
      "Canard",
      "Caribou",
      "Centaure",
      "Cerf",
      "Chameau",
      "Chat",
      "Cheval",
      "Chevre",
      "Chien",
      "Chimpanze",
      "Chinchilla",
      "Cigale",
      "Colvert",
      "Corbeau",
      "Coyote",
      "Crabe",
      "Criquet",
      "Crocodile",
      "Cygne",
      "Dauphin",
      "Dinosaure",
      "Dragon",
      "Ecureuil",
      "Elan",
      "Elephant",
      "Elfe",
      "Faucon",
      "Fourmi",
      "Fourmilier",
      "Frelon",
      "Furet",
      "Geai",
      "Girafe",
      "Gnou",
      "Gorille",
      "Grenouille",
      "Griffon",
      "Guepard",
      "Guepe",
      "Hamster",
      "Herisson",
      "Heron",
      "Hibou",
      "Hippopotame",
      "Kangourou",
      "Koala",
      "Lama",
      "Lamantin",
      "Lapin",
      "Leopard",
      "Lezard",
      "Licorne",
      "Lievre",
      "Lion",
      "Loup",
      "Loutre",
      "Lutin",
      "Mammouth",
      "Meduse",
      "Minotaure",
      "Moineau",
      "Morse",
      "Mouton",
      "Oie",
      "Ornithorynque",
      "Ours",
      "Palourde",
      "Panda",
      "Papillon",
      "Perroquet",
      "Pinson",
      "Poisson",
      "Poney",
      "Renard",
      "Renne",
      "Requin",
      "Rhinoceros",
      "Sauterelle",
      "Scarabee",
      "Serpent",
      "Souris",
      "Sphinx",
      "Tatou",
      "Tigre",
      "Tortue",
      "Triceratops",
      "Truite",
      "Vison",
      "Wapiti",
      "Yak",
      "Zebre"
    )
    def colors =
      Vector(
        "Acier",
        "Argent",
        "Azur",
        "Beige",
        "Blanc",
        "Bleu",
        "Bordeaux",
        "Bronze",
        "Brun",
        "Cerise",
        "Chataigne",
        "Cuivre",
        "Cyan",
        "Emeraude",
        "Fuschia",
        "Gris",
        "Indigo",
        "Jaune",
        "Lila",
        "Magenta",
        "Marine",
        "Menthe",
        "Mousse",
        "Noir",
        "Olive",
        "Or",
        "Orange",
        "Platine",
        "Prune",
        "Rose",
        "Rouge",
        "Rubis",
        "Safran",
        "Saphir",
        "Tomate",
        "Turquoise",
        "Vert",
        "Violet"
      )

  object pt:
    def pieces = Vector("Bispo", "Dama", "Peao", "Rei", "Cavalo", "Torre")
    def animals = Vector(
      "Abelha",
      "Alce",
      "Alpaca",
      "Antilope",
      "Arraia",
      "Avestruz",
      "Baleia",
      "Cachorro",
      "Camaleao",
      "Camelo",
      "Canguru",
      "Carneiro",
      "Cavalo",
      "Cervo",
      "Cisne",
      "Coala",
      "Coelho",
      "Coral",
      "Cordeiro",
      "Coruja",
      "Corvo",
      "Crocodilo",
      "Dromedario",
      "Elefante",
      "Falcao",
      "Foca",
      "Formiga",
      "Gaivota",
      "Galo",
      "Ganso",
      "Gato",
      "Girafa",
      "Golfinho",
      "Gorila",
      "Gralha",
      "Grifo",
      "Guaxinim",
      "Hamster",
      "Hiena",
      "Iaque",
      "Iguana",
      "Impala",
      "Jacare",
      "Jaguar",
      "Javali",
      "Leao",
      "Leopardo",
      "Lhama",
      "Lince",
      "Lobo",
      "Lontra",
      "Morcego",
      "Ornitorrinco",
      "Ovelha",
      "Panda",
      "Pantera",
      "Papagaio",
      "Pato",
      "Pinguim",
      "Pombo",
      "Raposa",
      "Rena",
      "Rinoceronte",
      "Sapo",
      "Sardinha",
      "Serpente",
      "Suricata",
      "Tatu",
      "Tigre",
      "Touro",
      "Tubarao",
      "Tucano",
      "Urso",
      "Zebra"
    )
    def colors = Vector(
      "Ameixa",
      "Amarelo",
      "Ambar",
      "Aqua",
      "Azul",
      "Bege",
      "Branco",
      "Bronzeado",
      "Carmesim",
      "Castanho",
      "Ciano",
      "Cobalto",
      "Cobre",
      "Coral",
      "Creme",
      "Cinza",
      "Dourado",
      "Esmeralda",
      "Fuscia",
      "Indigo",
      "Laranja",
      "Lilas",
      "Limao",
      "Magenta",
      "Marinho",
      "Marrom",
      "Menta",
      "Musgo",
      "Oliva",
      "Platina",
      "Prata",
      "Preto",
      "Rosa",
      "Roxo",
      "Rubi",
      "Safira",
      "Tan",
      "Tangerina",
      "Teal",
      "Tomate",
      "Turquesa",
      "Verde",
      "Vermelho",
      "Violeta",
      "Zinco"
    )
    def adjectives = Vector(
      "Agradavel",
      "Altruista",
      "Amavel",
      "Amigavel",
      "Amoroso",
      "Animado",
      "Animador",
      "Apreciado",
      "Apto",
      "Arrojado",
      "Atencioso",
      "Apressado",
      "Astuto",
      "Ativo",
      "Atraente",
      "Atraido",
      "Audacioso",
      "Audaz",
      "Autentico",
      "Aventureiro",
      "Aveludado",
      "Belo",
      "Bondoso",
      "Brilhante",
      "Brincalhao",
      "Brio",
      "Calmo",
      "Camarada",
      "Caridoso",
      "Capaz",
      "Carismatico",
      "Carinhoso",
      "Caro",
      "Cativante",
      "Cauteloso",
      "Cavalheiresco",
      "Certo",
      "Charmoso",
      "Chique",
      "Cientifico",
      "Civico",
      "Civilizado",
      "Classico",
      "Comico",
      "Compreensivo",
      "Comunicativo",
      "Competente",
      "Comprometido",
      "Confiante",
      "Consciente",
      "Considerado",
      "Corajoso",
      "Cordial",
      "Criterioso",
      "Criativo",
      "Curioso",
      "Decidido",
      "Delicado",
      "Determinado",
      "Digno",
      "Dedicado",
      "Dinamico",
      "Doce",
      "Educado",
      "Eficaz",
      "Eficiente",
      "Elogiado",
      "Empatico",
      "Elegante",
      "Empolgado",
      "Encantador",
      "Encantado",
      "Energetico",
      "Engajado",
      "Engracado",
      "Esperto",
      "Espontaneo",
      "Estiloso",
      "Exuberante",
      "Fabuloso",
      "Feliz",
      "Fiel",
      "Fino",
      "Firme",
      "Forte",
      "Generoso",
      "Gentil",
      "Gracioso",
      "Habilidoso",
      "Honesto",
      "Honrado",
      "Humilde",
      "Imparcial",
      "Impecavel",
      "Independente",
      "Indomavel",
      "Inteligente",
      "Intenso",
      "Interessante",
      "Leal",
      "Legal",
      "Lindo",
      "Magico",
      "Magnifico",
      "Maravilhoso",
      "Misterioso",
      "Natural",
      "Nobre",
      "Notavel",
      "Otimista",
      "Paciente",
      "Perceptivo",
      "Perspicaz",
      "Persuasivo",
      "Poderoso",
      "Popular",
      "Pratico",
      "Preciso",
      "Precioso",
      "Prestigioso",
      "Prestativo",
      "Proativo",
      "Protetor",
      "Prudente",
      "Puro",
      "Racional",
      "Radiante",
      "Rapido",
      "Realista",
      "Receptivo",
      "Reflexivo",
      "Refinado",
      "Relevante",
      "Respeitavel",
      "Respeitoso",
      "Resiliente",
      "Responsavel",
      "Sincero",
      "Simpatico",
      "Sociavel",
      "Solidario",
      "Sonhador",
      "Sorridente",
      "Talentoso",
      "Tranquilo",
      "Valente",
      "Vibrante",
      "Vigoroso",
      "Virtuoso",
      "Visionario"
    )
    def jobs = Vector(
      "Advogado",
      "Arqueologo",
      "Arquiteto",
      "Artista",
      "Atleta",
      "Astronauta",
      "Biologo",
      "Bombeiro",
      "Capitao",
      "Chefe",
      "Cientista",
      "Cirurgiao",
      "Comandante",
      "Conselheiro",
      "Cozinheiro",
      "Dentista",
      "Designer",
      "Eletricista",
      "Engenheiro",
      "Escritor",
      "Explorador",
      "Fotografo",
      "Geologo",
      "Historico",
      "Inventor",
      "Jornalista",
      "Juiz",
      "Lutador",
      "Mago",
      "Mecanico",
      "Medico",
      "Musico",
      "Navegador",
      "Ninja",
      "Piloto",
      "Pirata",
      "Professor",
      "Programador",
      "Psicologo",
      "Soldado",
      "Tecnico",
      "Viajante",
      "Vidente",
      "Vigilante"
    )

  object en:
    def colors =
      Vector(
        "Amber",
        "Aqua",
        "Azure",
        "Beige",
        "Black",
        "Blue",
        "Brass",
        "Brown",
        "Burgundy",
        "Cerulean",
        "Cherry",
        "Chestnut",
        "Cobalt",
        "Copper",
        "Crimson",
        "Cyan",
        "Emberald",
        "Fuschia",
        "Gold",
        "Gray",
        "Green",
        "Indigo",
        "Lilac",
        "Lime",
        "Magenta",
        "Maroon",
        "Mint",
        "Moss",
        "Navy",
        "Olive",
        "Orange",
        "Pink",
        "Platinum",
        "Plum",
        "Purple",
        "Red",
        "Rose",
        "Ruby",
        "Saffron",
        "Sapphire",
        "Silver",
        "Steel",
        "Tan",
        "Tangerine",
        "Teal",
        "Tomato",
        "Turquoise",
        "Violet",
        "White",
        "Yellow"
      )

    def pieces =
      Vector(
        "Bishop",
        "King",
        "Knight",
        "Pawn",
        "Queen",
        "Rook"
      )

    def animals =
      Vector(
        "Aardvark",
        "Alligator",
        "Alpaca",
        "Ant",
        "Antelope",
        "Anteater",
        "Armadillo",
        "Baboon",
        "Badger",
        "Bat",
        "Bear",
        "Bee",
        "Beetle",
        "Bison",
        "Buffalo",
        "Butterfly",
        "Camel",
        "Carabao",
        "Cariboo",
        "Cat",
        "Centaur",
        "Cheetah",
        "Chicken",
        "Chimpanzee",
        "Chinchilla",
        "Cicada",
        "Clam",
        "Cod",
        "Coyote",
        "Crab",
        "Cricket",
        "Crocodile",
        "Crow",
        "Deer",
        "Dinosaur",
        "Dog",
        "Dolphin",
        "Dragon",
        "Duck",
        "Eagle",
        "Echidna",
        "Eel",
        "Elephant",
        "Elf",
        "Elk",
        "Falcon",
        "Ferret",
        "Finch",
        "Fish",
        "Fly",
        "Fox",
        "Frog",
        "Gerbil",
        "Giraffe",
        "Gnu",
        "Goat",
        "Goblin",
        "Goldfish",
        "Goose",
        "Gorilla",
        "Grasshopper",
        "Grebe",
        "Griffin",
        "Grouse",
        "Guinea",
        "Hamster",
        "Hare",
        "Hawk",
        "Hedgehog",
        "Herring",
        "Heron",
        "Hippopotamus",
        "Hornet",
        "Horse",
        "Hound",
        "Hyena",
        "Impala",
        "Insect",
        "Jackal",
        "Jellyfish",
        "Kangaroo",
        "Kingfisher",
        "Kiwi",
        "Koala",
        "Lark",
        "Leopard",
        "Lion",
        "Lizard",
        "Llama",
        "Locust",
        "Louse",
        "Macaw",
        "Mallard",
        "Mammoth",
        "Manatee",
        "Marten",
        "Mink",
        "Minnow",
        "Minotaur",
        "Mole",
        "Monkey",
        "Moose",
        "Mosquito",
        "Mouse",
        "Mule",
        "Muskrat",
        "Octopus",
        "Otter",
        "Owl",
        "Ox",
        "Oyster",
        "Panda",
        "Parrot",
        "Penguin",
        "Platypus",
        "Pony",
        "Porcupine",
        "Prairie",
        "Pug",
        "Rabbit",
        "Raccoon",
        "Raven",
        "Reindeer",
        "Rhinoceros",
        "Salmon",
        "Sardine",
        "Scorpion",
        "Seal",
        "Serval",
        "Shark",
        "Sheep",
        "Snail",
        "Snake",
        "Sphinx",
        "Spider",
        "Squirrel",
        "Swan",
        "Termite",
        "Tiger",
        "Triceratops",
        "Troll",
        "Trout",
        "Turtle",
        "Unicorn",
        "Walrus",
        "Wasp",
        "Weasel",
        "Werewolf",
        "Whale",
        "Wolf",
        "Wombat",
        "Woodchuck",
        "Worm",
        "Wren",
        "Yak",
        "Yellowjacket",
        "Zebra"
      )

    def adjectives =
      Vector(
        "Abiding",
        "Able",
        "Absolute",
        "Absolved",
        "Abundant",
        "Academic",
        "Accepted",
        "Accurate",
        "Active",
        "Acute",
        "Adamant",
        "Adept",
        "Adequate",
        "Adjusted",
        "Admired",
        "Adonic",
        "Adorable",
        "Adored",
        "Adroit",
        "Advanced",
        "Affable",
        "Affined",
        "Affluent",
        "Ageless",
        "Agile",
        "Alert",
        "Alive",
        "Allied",
        "Alluring",
        "Alright",
        "Amative",
        "Amatory",
        "Amazing",
        "Amenable",
        "Amiable",
        "Amicable",
        "Amusing",
        "Angelic",
        "Aplenty",
        "Appetent",
        "Apposite",
        "Apropos",
        "Apt",
        "Ardent",
        "Artistic",
        "Aspirant",
        "Aspiring",
        "Assured",
        "Assuring",
        "Astir",
        "Astute",
        "Athletic",
        "Atypical",
        "August",
        "Avid",
        "Awaited",
        "Awake",
        "Aware",
        "Awash",
        "Awesome",
        "Balanced",
        "Baronial",
        "Beaming",
        "Beatific",
        "Becoming",
        "Beloved",
        "Benefic",
        "Benign",
        "Best",
        "Better",
        "Big",
        "Biggest",
        "Bijou",
        "Blazing",
        "Blessed",
        "Blissful",
        "Blithe",
        "Blooming",
        "Bold",
        "Bonny",
        "Bonzer",
        "Boss",
        "Bound",
        "Brainy",
        "Brave",
        "Brawny",
        "Breezy",
        "Brief",
        "Bright",
        "Brill",
        "Brimming",
        "Brisk",
        "Bubbly",
        "Budding",
        "Buff",
        "Bullish",
        "Buoyant",
        "Bustling",
        "Busy",
        "Calm",
        "Calming",
        "Canny",
        "Canty",
        "Capable",
        "Capital",
        "Carefree",
        "Careful",
        "Caring",
        "Casual",
        "Centered",
        "Central",
        "Cerebral",
        "Certain",
        "Champion",
        "Charming",
        "Cheerful",
        "Cherry",
        "Chic",
        "Chipper",
        "Chirpy",
        "Choice",
        "Chosen",
        "Chummy",
        "Civic",
        "Civil",
        "Classic",
        "Classy",
        "Clean",
        "Clear",
        "Clement",
        "Clever",
        "Close",
        "Clubby",
        "Coequal",
        "Cogent",
        "Coherent",
        "Colossal",
        "Coltish",
        "Comic",
        "Comical",
        "Complete",
        "Composed",
        "Concise",
        "Concrete",
        "Constant",
        "Content",
        "Cool",
        "Coolest",
        "Copious",
        "Cordial",
        "Correct",
        "Cosmic",
        "Cosy",
        "Courtly",
        "Cozy",
        "Creative",
        "Credible",
        "Crisp",
        "Crucial",
        "Cuddly",
        "Cultured",
        "Cunning",
        "Curious",
        "Current",
        "Cushy",
        "Cute",
        "Dainty",
        "Dandy",
        "Dapper",
        "Daring",
        "Darling",
        "Dashing",
        "Dazzling",
        "Dear",
        "Debonair",
        "Decent",
        "Deciding",
        "Decisive",
        "Decorous",
        "Deep",
        "Defiant",
        "Definite",
        "Deft",
        "Delicate",
        "Deluxe",
        "Designer",
        "Desired",
        "Desirous",
        "Destined",
        "Devoted",
        "Devout",
        "Didactic",
        "Diligent",
        "Dinkum",
        "Direct",
        "Discreet",
        "Discrete",
        "Distinct",
        "Diverse",
        "Divine",
        "Doable",
        "Dominant",
        "Doting",
        "Doughty",
        "Dreamy",
        "Driven",
        "Driving",
        "Durable",
        "Dutiful",
        "Dynamic",
        "Dynamite",
        "Eager",
        "Early",
        "Earnest",
        "Earthly",
        "Earthy",
        "Easy",
        "Eclectic",
        "Economic",
        "Ecstatic",
        "Edified",
        "Educated",
        "Elated",
        "Elating",
        "Elder",
        "Electric",
        "Elegant",
        "Eligible",
        "Eloquent",
        "Emerging",
        "Eminent",
        "Enamored",
        "Enduring",
        "Engaging",
        "Enhanced",
        "Enough",
        "Enticing",
        "Equable",
        "Equal",
        "Equipped",
        "Erudite",
        "Especial",
        "Esteemed",
        "Esthetic",
        "Eternal",
        "Ethical",
        "Euphoric",
        "Eventful",
        "Evident",
        "Exact",
        "Exalted",
        "Exotic",
        "Exultant",
        "Fab",
        "Fabulous",
        "Facile",
        "Factual",
        "Fain",
        "Fair",
        "Faithful",
        "Famed",
        "Familial",
        "Familiar",
        "Family",
        "Famous",
        "Fancy",
        "Fast",
        "Favored",
        "Favorite",
        "Fearless",
        "Feasible",
        "Fervent",
        "Festal",
        "Festive",
        "Fiery",
        "Fine",
        "Finer",
        "Finest",
        "Firm",
        "First",
        "Fit",
        "Fitting",
        "Flash",
        "Flashy",
        "Flawless",
        "Fleet",
        "Fluent",
        "Flying",
        "Focused",
        "Fond",
        "Forceful",
        "Foremost",
        "Forward",
        "Fragrant",
        "Frank",
        "Free",
        "Freely",
        "Fresh",
        "Friendly",
        "Fruitful",
        "Full",
        "Fun",
        "Funny",
        "Gainful",
        "Gallant",
        "Galore",
        "Game",
        "Gamesome",
        "Generous",
        "Genial",
        "Genteel",
        "Gentle",
        "Genuine",
        "Germane",
        "Gettable",
        "Giddy",
        "Gifted",
        "Giving",
        "Glad",
        "Gleaming",
        "Gleeful",
        "Glorious",
        "Glowing",
        "Godly",
        "Golden",
        "Good",
        "Goodly",
        "Gorgeous",
        "Graced",
        "Graceful",
        "Gracile",
        "Gracious",
        "Gradely",
        "Graithly",
        "Grand",
        "Grateful",
        "Great",
        "Greatest",
        "Groovy",
        "Grounded",
        "Growing",
        "Grown",
        "Guided",
        "Guiding",
        "Gutsy",
        "Halcyon",
        "Hale",
        "Hallowed",
        "Handsome",
        "Handy",
        "Happy",
        "Hardy",
        "Harmless",
        "Head",
        "Healing",
        "Healthy",
        "Hearty",
        "Heavenly",
        "Heedful",
        "Helpful",
        "Hep",
        "Heralded",
        "Heroic",
        "High",
        "Highest",
        "Hip",
        "Holy",
        "Homely",
        "Honest",
        "Honeyed",
        "Honorary",
        "Honored",
        "Hopeful",
        "Hotshot",
        "Huggy",
        "Humane",
        "Humble",
        "Humorous",
        "Hygienic",
        "Hypnotic",
        "Ideal",
        "Idolized",
        "Imitable",
        "Immense",
        "Immortal",
        "Immune",
        "Impish",
        "Improved",
        "Incisive",
        "Included",
        "Inerrant",
        "Infant",
        "Infinite",
        "Informed",
        "Initiate",
        "Innocent",
        "Inspired",
        "Integral",
        "Intense",
        "Intent",
        "Internal",
        "Intimate",
        "Intrepid",
        "Inviting",
        "Jaunty",
        "Jazzed",
        "Jazzy",
        "Jestful",
        "Jesting",
        "Jewelled",
        "Jiggish",
        "Jigjog",
        "Jocose",
        "Jocular",
        "Jocund",
        "Joint",
        "Jointed",
        "Jolly",
        "Jovial",
        "Joyful",
        "Joyous",
        "Joysome",
        "Jubilant",
        "Jump",
        "Just",
        "Keen",
        "Kempt",
        "Key",
        "Kind",
        "Kindly",
        "Kindred",
        "Kinetic",
        "Kingly",
        "Knightly",
        "Knowable",
        "Knowing",
        "Kooky",
        "Kosher",
        "Large",
        "Lasting",
        "Laudable",
        "Laureate",
        "Lavish",
        "Lawful",
        "Leading",
        "Learned",
        "Legal",
        "Legible",
        "Legit",
        "Leisured",
        "Leonine",
        "Lepid",
        "Lettered",
        "Liberal",
        "Lightly",
        "Likable",
        "Like",
        "Liked",
        "Likely",
        "Limber",
        "Literary",
        "Literate",
        "Lithe",
        "Live",
        "Lively",
        "Logical",
        "Lordly",
        "Lovable",
        "Loved",
        "Lovely",
        "Loving",
        "Loyal",
        "Lucent",
        "Lucid",
        "Lucky",
        "Luminous",
        "Luscious",
        "Lush",
        "Lustrous",
        "Magical",
        "Magnetic",
        "Main",
        "Majestic",
        "Major",
        "Manifest",
        "Manly",
        "Mannerly",
        "Many",
        "Marked",
        "Master",
        "Masterly",
        "Maternal",
        "Mature",
        "Maturing",
        "Maximal",
        "Mediate",
        "Meek",
        "Mellow",
        "Merciful",
        "Merry",
        "Meteoric",
        "Mighty",
        "Mindful",
        "Minikin",
        "Mint",
        "Mirthful",
        "Model",
        "Modern",
        "Modest",
        "Moneyed",
        "Moral",
        "More",
        "Most",
        "Motor",
        "Moving",
        "Much",
        "Mucho",
        "Muscular",
        "Musical",
        "Must",
        "Mutual",
        "National",
        "Native",
        "Natty",
        "Natural",
        "Nearby",
        "Neat",
        "Needed",
        "Neoteric",
        "Nestling",
        "New",
        "Newborn",
        "Nice",
        "Nifty",
        "Nimble",
        "Nippy",
        "Noble",
        "Noetic",
        "Normal",
        "Notable",
        "Noted",
        "Novel",
        "Now",
        "Official",
        "Olympian",
        "Once",
        "One",
        "Onward",
        "Open",
        "Optimal",
        "Optimum",
        "Opulent",
        "Orderly",
        "Organic",
        "Oriented",
        "Original",
        "Outgoing",
        "Overt",
        "Pally",
        "Palpable",
        "Parental",
        "Partisan",
        "Paternal",
        "Patient",
        "Peaceful",
        "Peachy",
        "Peerless",
        "Peppy",
        "Perfect",
        "Perky",
        "Pet",
        "Petite",
        "Picked",
        "Pierian",
        "Pilot",
        "Pious",
        "Piquant",
        "Pithy",
        "Pivotal",
        "Placid",
        "Playful",
        "Pleasant",
        "Pleased",
        "Pleasing",
        "Plenary",
        "Plenty",
        "Plucky",
        "Plummy",
        "Plus",
        "Plush",
        "Poetic",
        "Pog",
        "Poignant",
        "Poised",
        "Polished",
        "Polite",
        "Popular",
        "Posh",
        "Positive",
        "Possible",
        "Potent",
        "Powerful",
        "Precious",
        "Precise",
        "Premier",
        "Premium",
        "Prepared",
        "Present",
        "Pretty",
        "Primal",
        "Primary",
        "Prime",
        "Primed",
        "Primo",
        "Princely",
        "Pristine",
        "Prize",
        "Prized",
        "Pro",
        "Probable",
        "Profound",
        "Profuse",
        "Prolific",
        "Prompt",
        "Proper",
        "Protean",
        "Proud",
        "Prudent",
        "Puissant",
        "Pukka",
        "Punchy",
        "Punctual",
        "Pure",
        "Quaint",
        "Quality",
        "Queenly",
        "Quick",
        "Quiet",
        "Quirky",
        "Quiver",
        "Quixotic",
        "Quotable",
        "Rad",
        "Radiant",
        "Rapid",
        "Rational",
        "Ready",
        "Real",
        "Refined",
        "Regal",
        "Regnant",
        "Regular",
        "Relaxed",
        "Relevant",
        "Reliable",
        "Relieved",
        "Renowned",
        "Resolute",
        "Resolved",
        "Restful",
        "Revered",
        "Reverent",
        "Rich",
        "Right",
        "Rightful",
        "Risible",
        "Robust",
        "Romantic",
        "Rooted",
        "Rosy",
        "Round",
        "Rounded",
        "Rousing",
        "Rugged",
        "Ruling",
        "Running",
        "Sacred",
        "Safe",
        "Sage",
        "Saintly",
        "Salient",
        "Salutary",
        "Sanguine",
        "Sapid",
        "Sapient",
        "Saucy",
        "Saving",
        "Savory",
        "Savvy",
        "Scenic",
        "Seamless",
        "Seasonal",
        "Seasoned",
        "Secure",
        "Sedulous",
        "Seemly",
        "Select",
        "Selfless",
        "Sensible",
        "Serene",
        "Service",
        "Set",
        "Settled",
        "Sharp",
        "Sheen",
        "Shining",
        "Shiny",
        "Showy",
        "Shrewd",
        "Sightly",
        "Silken",
        "Silky",
        "Silver",
        "Silvery",
        "Simple",
        "Sincere",
        "Sinewy",
        "Singular",
        "Sisterly",
        "Sizable",
        "Sizzling",
        "Skilled",
        "Skillful",
        "Sleek",
        "Slick",
        "Slinky",
        "Smacking",
        "Smart",
        "Smashing",
        "Smiley",
        "Smooth",
        "Snap",
        "Snappy",
        "Snazzy",
        "Snod",
        "Snug",
        "Soaring",
        "Sociable",
        "Social",
        "Societal",
        "Soft",
        "Soigne",
        "Solid",
        "Sonsy",
        "Sooth",
        "Soothing",
        "Soulful",
        "Sound",
        "Spacious",
        "Spangly",
        "Sparkly",
        "Special",
        "Specular",
        "Speedy",
        "Spicy",
        "Spiffy",
        "Spirited",
        "Splendid",
        "Sport",
        "Sporting",
        "Sportive",
        "Sporty",
        "Spot",
        "Spotless",
        "Spruce",
        "Spry",
        "Spunky",
        "Square",
        "Stable",
        "Stacked",
        "Stalwart",
        "Staminal",
        "Standard",
        "Standing",
        "Star",
        "Starry",
        "State",
        "Stately",
        "Staunch",
        "Steady",
        "Stellar",
        "Sterling",
        "Sthenic",
        "Stirred",
        "Stirring",
        "Stocky",
        "Stoical",
        "Storied",
        "Stout",
        "Striking",
        "Strong",
        "Studious",
        "Stunning",
        "Sturdy",
        "Stylish",
        "Suasive",
        "Suave",
        "Sublime",
        "Substant",
        "Subtle",
        "Succinct",
        "Sugary",
        "Suitable",
        "Sultry",
        "Summery",
        "Sunny",
        "Super",
        "Superb",
        "Superior",
        "Supernal",
        "Supple",
        "Supreme",
        "Sure",
        "Svelte",
        "Swank",
        "Sweet",
        "Swell",
        "Swift",
        "Swish",
        "Sylvan",
        "Tactful",
        "Talented",
        "Tangible",
        "Tasteful",
        "Tasty",
        "Teeming",
        "Tempean",
        "Tenable",
        "Terrific",
        "Thankful",
        "Thorough",
        "Thrilled",
        "Thriving",
        "Tidy",
        "Timeless",
        "Timely",
        "Tiptop",
        "Tireless",
        "Titanic",
        "Today",
        "Together",
        "Tolerant",
        "Top",
        "Tops",
        "Total",
        "Touching",
        "Tough",
        "Tranquil",
        "Traveled",
        "Tretis",
        "Trim",
        "True",
        "Trustful",
        "Trusting",
        "Trusty",
        "Truthful",
        "Tubular",
        "Tuneful",
        "Tympanic",
        "Uber",
        "Ultimate",
        "Ultra",
        "Unafraid",
        "Unbeaten",
        "Unbiased",
        "Unbroken",
        "Uncommon",
        "Unerring",
        "Unharmed",
        "Unhurt",
        "Unified",
        "Unique",
        "United",
        "Unshaken",
        "Unspoilt",
        "Untiring",
        "Unusual",
        "Upbeat",
        "Upcoming",
        "Uplifted",
        "Upright",
        "Upward",
        "Upwardly",
        "Urbane",
        "Usable",
        "Useful",
        "Utmost",
        "Valiant",
        "Valid",
        "Valorous",
        "Valuable",
        "Valued",
        "Vast",
        "Vaulting",
        "Vehement",
        "Venust",
        "Verified",
        "Versed",
        "Very",
        "Vestal",
        "Veteran",
        "Viable",
        "Vibrant",
        "Victor",
        "Vigilant",
        "Vigorous",
        "Virtuous",
        "Vital",
        "Vivid",
        "Vocal",
        "Volant",
        "Wanted",
        "Warm",
        "Wealthy",
        "Weighty",
        "Welcome",
        "Welcomed",
        "Well",
        "Welsome",
        "Whole",
        "Whopping",
        "Willed",
        "Willing",
        "Winged",
        "Winning",
        "Winsome",
        "Wired",
        "Wise",
        "Witty",
        "Wizard",
        "Wizardly",
        "Won",
        "Wondrous",
        "Workable",
        "Worldly",
        "Worth",
        "Worthy",
        "Xenial",
        "Yern",
        "Young",
        "Youthful",
        "Zaftig",
        "Zany",
        "Zappy",
        "Zazzy",
        "Zealed",
        "Zealful",
        "Zealous",
        "Zestful",
        "Zesty",
        "Zingy",
        "Zippy",
        "Zooty"
      )

    def jobs =
      Vector(
        "Actor",
        "Architect",
        "Artist",
        "Astronaut",
        "Astronomer",
        "Author",
        "Baker",
        "Banjoist",
        "Barber",
        "Bassist",
        "Beekeeper",
        "Blacksmith",
        "Botanist",
        "Builder",
        "Chef",
        "Chemist",
        "Choreographer",
        "Coder",
        "Dancer",
        "Dentist",
        "Diver",
        "Doctor",
        "Drummer",
        "Duelist",
        "Fan",
        "Farmer",
        "Florist",
        "Flutist",
        "Gardener",
        "Geologist",
        "Guide",
        "Guitarist",
        "Harpist",
        "Journalist",
        "Judge",
        "Lawyer",
        "Musician",
        "Ninja",
        "Nurse",
        "Officer",
        "Optician",
        "Pianist",
        "Player",
        "Runner",
        "Singer",
        "Trumpeter",
        "Ukulelist",
        "Vet",
        "Violist",
        "Wizard",
        "Writer"
      )
