...
trait Common {
  @inline implicit def toOrnicarFunctor[M[_]: Functor, A](fa: M[A]) =
    new ornicarFunctor(fa)
  @inline implicit def toOrnicarAddKcombinator[A](any: A) =
    new ornicarAddKcombinator(any)
  @inline implicit def toOrnicarRichMap[A, B](m: Map[A, B]) =
    new ornicarRichMap(m)
  @inline implicit def toOrnicarRichIdentity[A](a: A) =
    new ornicarRichIdentity(a)
}

final class ornicarFunctor[M[_]: Functor, A](fa: M[A]) {
  def map2[N[_], B, C](
      f: B => C
  )(implicit m: A <:< N[B], f1: Functor[M], f2: Functor[N]): M[N[C]] =
    f1.map(fa) { k =>
      f2.map(k: N[B])(f)
    }
}
